...
/*
 ...
 whitespace padding?  spec says to leave it ( i say we get rid of it, if it's in every line )
 extra comma at the end of each row?  spec says it shouldn't be there ( i say we get rid of it, if it's on every line )

  add utf-16 truncation warning
  tests against row 1 if no header
...
int get_row(struct sRow * row, char * string, char delimiter, int quote)
{
